...
boxplot(x=model_boxplot, col=seasons_colors, main="Biochemical Shannon Diversity", xlab="Seasons", ylab="Shannon Diversity")
#text(1:length(seasons_names), par("usr")[3]-(par("usr")[4]-par("usr")[3])/14, srt=-22.5, adj=0.5, labels=seasons_names, xpd=TRUE, cex=0.9)
div_tukey <- seasons.tukey.test(model = model_div$shannon ~ seasons)
